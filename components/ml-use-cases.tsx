...
const UseCase = ({ title, icon, content }: UseCaseProps) => {
  ...
  return (
    <div className="border border-neutral-800 rounded-sm overflow-hidden mb-4 bg-black/50 backdrop-blur-sm">
      <button
      ...
      <AnimatePresence>
        {isOpen && (
          ...
          >
            <div className="p-6 pt-4 border-t border-neutral-800 text-neutral-300">{content}</div>
          </motion.div>
        )}
      </AnimatePresence>
    </div>
...
export default function MLUseCases() {
  ...
  return (
    <div className="w-full">
      {/* Use a single column layout to ensure one use case per line */}
